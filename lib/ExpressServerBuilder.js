...
class ExpressServerBuilder {
    ...
    async create(routes) {
        try {
            console.log(`✔  ${this.fileName}.js file created.`)
            const content = this.parseContent(routes)
            await this.createFile(this.fileName, content)
        } catch (e) {
            console.log(`Something went wrong creating "${this.fileName}" file`, e)
        }
    }
...
`
    ...
    parseRequirements(routes) {
        // include index route
        let content = `const indexRouter = require('${this.routesPath}/index')\n`
        // include each route
    ...
    parseUseExpressions(routes) {
        // use index route
        let content = "app.use('/', indexRouter)\n"
        // use each route
        for (const route in routes) {
            content += `app.use('/${route}',${route}Router)\n`
        }
