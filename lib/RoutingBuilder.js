...
class RoutingBuilder {
    ...
    parseRouteContent(tableName, tableObj, foreignKeys) {
        ...
        const postContent = this.parsePostContent(tableName, tableObj)
        const putContent = this.parsePutContent(tableName, tableObj)
...
`
     ...
     * TODO: handle relations 
     */
    parsePostContent(tableName, tableObj) {
        let tableFields = ''
        let tableAssignments = ''
        for (const tableAttr in tableObj) {
            if (tableAttr === 'id')
                continue // double definition of id creates an error
            tableFields += `
            ${tableAttr},`
            tableAssignments += `
                ${tableAttr}: ${tableAttr},`
...
router.post('/', async function (req, res, next) {
    try {
        ...
        const ${tableName} = await model.${tableName}.create({${tableAssignments}
        })
        res.status(201).json({
            data: ${tableName},
...
`
     ...
     * TODO: handle relations 
     */
    parsePutContent(tableName, tableObj) {
        let tableFields = ''
        let tableAssignments = ''
        for (const tableAttr in tableObj) {
            if (tableAttr === 'id')
...
router.put('/:id', async function (req, res, next) {
    try {
        ...
        const result = await model.${tableName}.update({${tableAssignments}
        }, {
            where: {
                id: id
            }
