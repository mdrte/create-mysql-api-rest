...
class ModelBuilder {
  ...
  async create(tableName, tableObj, foreignKeys) {
    try {
      const content = this.parseModelContent(tableName, tableObj, foreignKeys)
   ...
   * TODO: continue 
   */
  parseModelContent(tableName, tableObj, foreignKeys) {
    ...
    fields.forEach((field) => {
      text += this.parseField(field, tableObj[field], foreignKeys)
    });
    ...
    text += ", {\n";
    text += spaces + spaces + `timestamps: ${this.timestamp},\n`
   ...
   * TODO: continue 
   */
  parseField(fieldName, fieldDefinition, foreignKeys) {
    const spaces = " ".repeat(this.indentation)
    ...
    // Find foreign key
    var foreignKey = foreignKeys[fieldName] ? foreignKeys[fieldName] : null
    ...
    // parse each attr of the field
    _.each(fieldAttr, function (attr) {
      const isSerialKey = fieldDefinition.foreignKey && tableUtils.isSerialKey(fieldDefinition.foreignKey)
      switch (attr) {
        case "foreignKey":
          if (isSerialKey) {
            ...
            console.log(`   has 1 relation with ${fieldDefinition[attr].foreignSources.target_table}`)
          } else return true
          break
        case "primaryKey":
        ...
        case "autoIncrement":
          return true
        default:
          if (attr === "comment" && fieldDefinition[attr] === null) {
            return true
          }
  ...
  parseTypeAttr(attributte) {
    ...
    if (_attr === "boolean" || _attr === "bit(1)" || _attr === "bit") {
      ...
      val = _.isBoolean(attributte) ? attributte : `'${attributte}'`
    }
    return val
  }
  ...
  /**
   * Verifies that all rules applied for a dependency table are fulfilled
   * @param {string} tableName 
   * @param {object} tableObj
   * @returns {boolean} 
   */
  isValidDependency(tableName, tableObj) {
    // All tables are written in lowercase
    if (!tableUtils.isLowerCase(tableName))
      return false
    // all valid tables end plural
    if (!pluralize.isPlural(tableName))
      return false
    // all models have a primary key called id 
    if (tableObj.id === undefined || tableObj.id.primaryKey !== true)
      return false

    // All tables that represent a relation must have as a suffix the model's name in singular
    const words = tableName.split('_')
    if (words.length > 1 && pluralize.isSingular(words[0]))
      return true
